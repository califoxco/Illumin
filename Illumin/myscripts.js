...
//firebase
var myDataRef = new Firebase('https://glaring-inferno-4068.firebaseio.com/');
var message = {"left":false, "right":false, "top":false, "down":false};
...
myDataRef.child("keys").on('child_added', function(snapshot) {
...
function tick(time, levels) {
    ...
    if ((38 in keysDown || 13 in keysDown || message.top) && Gracount == 0) { //up
        up = true;
        if (jumtimes == 0) jumtimes += 40;
    }
    if (40 in keysDown || message.down) { //down
        var hitType = 0;
    ...
    if (37 in keysDown || goingLeft || message.left) { //left
        charStat = 2;
    ...
    if (39 in keysDown || goingRight || message.right) { //right
        charStat = 1;

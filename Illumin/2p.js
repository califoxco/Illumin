...
characterImg.src = "charSheet3.png";
...
function drawLevel(obstacles, levels){
    ...
    for (var i = Math.floor(runLevel.player1[1]) - 12; i <= Math.floor(runLevel.player1[1]) + 12; i++){
        for (var j = Math.floor(runLevel.player1[0]) - 16; j <= Math.floor(runLevel.player1[0]) + 16; j++){
    ...
    for (var x = 0; x < obstaclesList.length; x++) { 
        var drawX = obstaclesList[x][0] - (runLevel.player1[0] - 9);
        var drawY = obstaclesList[x][1] - (runLevel.player1[1] - 12);
    ...
    for (var x = 0; x < artifactList2.length; x++){
        var drawX = artifactList[x][0] - (runLevel.player1[0] - 9);
        var drawY = artifactList[x][1] - (runLevel.player1[1] - 12);
...
function drawPlayer(player1){
    if (charStat == 1) {cycle = Math.floor((cycleCount % 16)/4)+1; cycleCount++}
    else if (charStat == 2)  { cycle = Math.floor((cycleCount % 16)/4)+ 5; cycleCount++}
    else {cycle = 0 ;cycleCount = 0 ;}
    cx.drawImage(characterImg,  cycle * charWidth, 0, charWidth, charHeight, scale*(9),  scale*(12), charWidth, charHeight);    
}
...
function tick(time, levels) {
    ...
    var downHit = tryhit((runLevel.player1[0]), (runLevel.player1[1] + (Gracount * time * gravity)+1.08));
        
    if (38 in keysDown && Gracount == 0) { //up
    ...
    if (40 in keysDown) { //down
        ...
        runLevel.player1[1] += gravity*time;
        } 
    }
    if (37 in keysDown) { //left
        charStat = 2;
        var leftHit1 = tryhit((runLevel.player1[0] - speed * time), (runLevel.player1[1]));
        var leftHit2 = tryhit((runLevel.player1[0] - speed * time), (runLevel.player1[1]+1));
        var leftHit3 = tryhit((runLevel.player1[0] - speed * time), (runLevel.player1[1])-0.7);
        var leftHit4 = tryhit((runLevel.player1[0] - speed * time), (runLevel.player1[1]+1)-0.7);
        var leftHit5 = tryhit((runLevel.player1[0] - speed * time), (runLevel.player1[1]));
        var leftHit6 = tryhit((runLevel.player1[0] - speed * time), (runLevel.player1[1]+1));
        var leftHit7 = tryhit((runLevel.player1[0] - speed * time), (runLevel.player1[1])-0.7);
        var leftHit8 = tryhit((runLevel.player1[0] - speed * time), (runLevel.player1[1]+1)-0.7);
        if (!leftHit1[0] && !leftHit2[0] && !leftHit3[0] &&!leftHit4[0]) {runLevel.player1[0] -= speed *time;
        cloud -= 0.6;}
        else {
            var hitType;
        ...
        if (hitType == 5) { runLevel.player1[0] -= speed *time; cloud -= 0.6;} 
        }
    }
    if (39 in keysDown) { //right
        charStat = 1;
        var rightHit1 = tryhit((runLevel.player1[0] + speed * time), (runLevel.player1[1]));
        var rightHit2 = tryhit((runLevel.player1[0] + speed * time), (runLevel.player1[1]+1));
        var rightHit3 = tryhit((runLevel.player1[0] + speed * time), (runLevel.player1[1])-0.7);
        var rightHit4 = tryhit((runLevel.player1[0] + speed * time), (runLevel.player1[1]+1)-0.7);
        var rightHit5 = tryhit((runLevel.player1[0] + speed * time)+1, (runLevel.player1[1]));
        var rightHit6 = tryhit((runLevel.player1[0] + speed * time)+1, (runLevel.player1[1]+1));
        var rightHit7 = tryhit((runLevel.player1[0] + speed * time)+1, (runLevel.player1[1])-0.7);
        var rightHit8 = tryhit((runLevel.player1[0] + speed * time)+1, (runLevel.player1[1]+1)-0.7);
        if (!rightHit1[0] && !rightHit2[0] && !rightHit3[0] && !rightHit4[0]){ runLevel.player1[0] += speed *time;
        cloud += 0.6;}
        else{
            var hitType;
        ...
        if (hitType == 5 || hitType==undefined) { runLevel.player1[0] += speed *time; cloud += 0.6;} 
        }
    }
    ...
    if (!downHit[0]){
        runLevel.player1[1] += gravity*time*Gracount;
        Gracount += 0.05;
    } 
    else {
    ...
    //jumps
    if (jumtimes >0){
        var topHit = tryhit((runLevel.player1[0]), (runLevel.player1[1] - (Gracount * time * jumcount)-1));
        var hitType = 0;
        if (topHit[1] != undefined) hitType = levelPlan[levelNum][topHit[2]][topHit[1]];
        if (hitType ==0){
            jumtimes--;
            runLevel.player1[1] -= gravity*time*jumcount;
            if (jumcount >= 0.05)jumcount -= 0.03;
        } 
        else if (hitType == 5 )  {
            runLevel.player1[1] -= speed*time;
            jumtimes = 0;
            jumcount = 1.80;
        }
        else {
    ...
    //artifact check
    for (var x = 0; x < artifactList2.length; x++){
        if (runLevel.player1[0] < artifactList2[x][0] + 1 &&
            runLevel.player1[0] + 1 > artifactList2[x][0] &&
            runLevel.player1[1] < artifactList2[x][1] + 1 &&
            2 + runLevel.player1[1] > artifactList2[x][1]){
            artifacts--;
    ...
    if (runLevel.player1[1] > runLevel.height+10) death = true;
    }
    
    document.getElementById('artifact').innerHTML = "Artifacts to collect: " + artifacts;
    if (levelNum == 8){
        if (runLevel.player1[0]>7 && runLevel.player1[0] <16 && runLevel.player1[1] >55 && runLevel.player1[1]<57) document.getElementById('laugh').play();
    }
}
...
function checkDeath(){
    if (death) {
        ...
        runLevel.player1[0] = spawn[0];
        runLevel.player1[1] = spawn[1];
        art();
        artifacts = artifactIndex.length;
        death = false;
    }
